...
%% initialization 
K = 3;%first two: legitimate users
%third eavesdropping user
max_dist     = 100;%meters
max_eta      = 10;
etath        = 4;%change this 
noisepower   = 0.1;
...
%change here
for d = 2: K-1
    power_vec(d) = power_vec(d-1)/10^(0.1);
end

power_vec(3) = (alpha1_vec(cases))*10^(transmit_snrdb_vec(1)/10)*noisepower;
...
cvx_begin quiet
   ...
   minimize (-decision_uk'*K_vec)%total legitimate and eavesdropper
   subject to
      var1 : -sum(decision_uk)+ sum(decision_uk.^2)<=0;
      var2 : decision_uk(1:K).*((noisepower^2 + interf_vec(1:K) +...
          power_vec(1:K).*mean(g_vec(1:K,:),2))...
                -power_vec(1:K).*mean(g_vec(1:K,:),2)*...
                (1+1/sinr_thmin))- (decision_uk(K).*((noisepower^2 +...
                interf_vec(K) +...
          power_vec(K).*mean(g_vec(K,:),2))...
                -power_vec(K).*mean(g_vec(K,:),2)*...
                (1+1/sinr_thmax))) <= 0
      var3: decision_uk >=0 
...
decision_uk = decision_uk>0

for r = 1:K
    rate_vec(r,1) = power_vec(r)*mean(g_vec(r,:),2)/...
        (noisepower^2 + interf_vec(r));
end

cs(cases,t) = (log((1+sum(decision_uk.*power_vec(1:K).*mean(g_vec(1:K,:),2)./...
    (noisepower^2 + (interf_vec)))))...
       -log(1+sum(decision_uk(K).*power_vec(K).*mean(g_vec(K,:),2)./...
       (noisepower^2 + (interf_vec)))));
...
legend({'\alpha_{{e}_1} = 0.001';'\alpha_{{e}_2} = 0.01';'\alpha_{{e}_3} = 0.1';'\alpha_{{e}_4} = 0.5';})