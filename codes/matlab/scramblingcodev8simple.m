...
[noise] = awgnNoiseAddition(data_channel1);
...
for i = 1:length(snr)
    ...
    y = noise + (sqrt(1)*10^(-snr(i)/20))*noise; %Addition of Noise
...
[data_channel3] = rayleighFading(N,tx_user1_hop2,n);
[data_channel4] = rayleighFading(N,tx_user2,n);

%----------------------superposition of the messages of different users---
%transmit based on time slot
%time slot 1
data_channel2=data_channel3+data_channel4;

%------------------------Addition of AWGN noise ---------------------------
[noise] = awgnNoiseAddition(data_channel2);
...
[SBer3,est3] = berCompute(data_equilized3,data_user2,code3,N,i);
