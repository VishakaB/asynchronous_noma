...
delta_mat = zeros(K,K);
delta_mat(1,:) = zeros(K,1);
delta_mat(2:K,:) = 0.01*ones(K-1,K);%B1,... Bn, C1....,Cn, ....... %Z1,....Zn

reverse_delta_mat(K,:)  = zeros(K,1);
reverse_delta_mat(1:K-1,:) = 0.01*ones(K-1,K);%A1, A2
...
elseif (user_strength(i) > 1 &j >1 & i==round(length(user_strength)))
    ...
    p_interb1 = (1/(timeoff_max -timeoff_min))*mean(q*sum(delta_i));
    ...
    p_bitb2 = 1/4*avgperr2/log(mod_order); 
    ber_vec(i,j) = p_bitb2;
