...
receive_pow_ratio_vec = [1,2,5];
u = 1;%initiate u
nbiter = 10;

for receive_pow_ratio = 1: 3
for j = 1:nbiter
rng(j)%random seed
...
delta_mat = zeros(K,K);
delta_mat(1:K,:) = (1-0.01*power_vec(1:K)).*abs(time_offset*randn(K,K));%B1,... Bn, C1....,Cn, ....... %Z1,....Zn
...
figure (1)
semilogy(receive_pow_ratio_vec,smooth(smooth(avgavgber_1)),'bo--')
hold on 
semilogy(receive_pow_ratio_vec,smooth(smooth(avgavgber_2)),'rs-')
hold on 
semilogy(receive_pow_ratio_vec,smooth(smooth(avgavgber_3)),'k+--')
