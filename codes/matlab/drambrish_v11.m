...
cvx_begin quiet
   ...
   minimize (-((log((1+sum(decision_uk.*power_vec(1:K).*mean(g_vec(1:K,:),2)./(noisepower^2 + (interf_vec))))))))
   %...
       %-log(1+sum(decision_uk.*power_vec_e(1:K).*mean(g_vec_e(1:K,:),2)./(noisepower^2 + (interf_vec))))))))%maximize secrecy
   subject to
