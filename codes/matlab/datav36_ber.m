...
snr_vec =linspace(60,100,5);%change here
mpriority = length(snr_vec);
x = zeros(mpriority,1);
y = zeros(mpriority,1);
z = zeros(mpriority,1);
...
r = zeros(mpriority,1);
s = zeros(mpriority,1);
weakestconv = zeros(mpriority,1);
weakestprop = zeros(mpriority,1);
...
[x(indx),y(indx),z(indx),zz(indx),e(indx),f(indx),g(indx),...
    h(indx),i(indx),j(indx),r(indx),s(indx),weakestconv(indx),...
    weakestprop(indx),intermconv(indx),intermprop(indx)] =...
...
function [a,b,c,d,e,f,g,h,i,j,r,s,weakestconv,weakestprop,intermconv,intermprop] =...
    seqsic(initialK,alldatadecoded,K,priority,N,snr)
...
noisepower = sum(power_vec)/10^snr;
...
weakestconv = mean(avgberweakconv);
weakestprop = mean(avgberweakprop);
...
elseif (user_strength(i) > 1 & j == 1 & i~=length(user_strength))%B1
    ...
    p_err_sym3 = p_interb1*(ber_vec(i-1,j)*ber_vec(i-1,j+1)+...
        (1-ber_vec(i-1,j))*ber_vec(i-1,j+1)+(ber_vec(i-1,j))*(1-ber_vec(i-1,j+1))...
        +(1-ber_vec(i-1,j))*(1-ber_vec(i-1,j+1)))*log(mod_order);
...
elseif (user_strength(i) > 1 & j > 1 & i~=length(user_strength))%B2.....Bn & not last user 
    ...
    p_err_sym4 = p_interb2*(ber_vec(i-1,j)*ber_vec(i-1,j+1)+...
        (1-ber_vec(i-1,j))*ber_vec(i-1,j+1)+(ber_vec(i-1,j))*...
        (1-ber_vec(i-1,j+1))...
        +(1-ber_vec(i-1,j))*(1-ber_vec(i-1,j+1)))*log(mod_order);
    p_bitb2 = p_err_sym4/log(mod_order);
    ber_vec(i,j) = p_bitb2;
...
elseif (user_strength(i) > 1 & j >= 1 & i==length(user_strength))
    ...
    if (i>=3)
        p_iss    = power_vec(i-2);%interferes power
        delta_i =[(reverse_delta_mat(user_strength(i-2),j)); ...
        1-reverse_delta_mat(user_strength(i-2),j+1); 1-reverse_delta_mat(user_strength(i-2),j+2);...
        ...
        ber_vec(i-2,j)*(1 -ber_vec(i-2,j+1))*ber_vec(i-2,j+2)+...
        ber_vec(i-2,j)*(1-ber_vec(i-2,j+1))*(1-ber_vec(i-2,j+2))+...
        (1-ber_vec(i-2,j))*(ber_vec(i-2,j+1))*(ber_vec(i-2,j+2))+...
        (1-ber_vec(i-2,j))*(ber_vec(i-2,j+1))*(1-ber_vec(i-2,j+2))+...
        (1-ber_vec(i-2,j))*(1-ber_vec(i-2,j+1))*(ber_vec(i-2,j+2))+...
        (1-ber_vec(i-2,j))*(1-ber_vec(i-2,j+1))*(1-ber_vec(i-2,j+2));
    else
       A_branch = 1;
    end
    
    p_err_sym5 = p_interb3*A_branch*(ber_vec(i-1,j)*ber_vec(i-1,j+1)+...
        (1-ber_vec(i-1,j))*ber_vec(i-1,j+1)+(ber_vec(i-1,j))*(1-ber_vec(i-1,j+1))...
        +(1-ber_vec(i-1,j))*(1-ber_vec(i-1,j+1)))*log(mod_order);
    p_bitc2 = p_err_sym5/log(mod_order);
    ber_vec(i,j) = p_bitc2;
else
    %disp('oops')
end
