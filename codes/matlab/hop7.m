...
%BER of data user E
recdata7F=(data_equilizedF'*code8')';
xF_hat = ones(1,N);
xF_hat(recdata7F<0) = -1;
recdata8user=(data_equilized7'*code8'-xF_hat')';
est8h7=real(recdata8user)>0;
errors_user7(i) = size(find([data_user8- est8h7]),2); %Errors for User1
SBer8h7= errors_user7/N;   

x8user_hat = ones(1,N);
x8user_hat(recdata8user<0) = -1;
recdataF=(data_equilizedF'*code8'-x8user_hat')';
