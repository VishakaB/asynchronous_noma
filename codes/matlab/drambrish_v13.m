...
cvx_begin quiet
   ...
   minimize(-(sum(decision_uk.*power_vec(1:K).*mean(g_vec(1:K,:),2)./...
    (noisepower^2 + (interf_vec))))+sum(power_vec_e(1:K).*mean(g_vec_e(1:K,:),2)./...
       (noisepower^2 + (interf_vec))))
   subject to
      %var1: decision_uk.*(1+(power_vec(1:K).*mean(g_vec(1:K,:),2))./...
       %   (noisepower^2 + interf_vec)-...
        %  2^(1)/1*(1+(power_vec_e(1:K).*mean(g_vec_e(1:K,:),2))./...
         % (noisepower^2 + interf_vec))) >= 0 
