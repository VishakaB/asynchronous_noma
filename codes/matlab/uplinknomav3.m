...
eta1 = 4;	%Path loss exponent
eta2 = 1;
eta3 = 0.5;

%Generate Rayleigh fading channel for the three users
h1 = sqrt(d1^-eta1)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
h2 = sqrt(d2^-eta2)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
h3 = sqrt(d3^-eta3)*(randn(N/2,1) + 1i*randn(N/2,1))/sqrt(2);
...
for u = 1:length(Pt)
   %Received signals
   y = sqrt(Pt(u))*h1.*xmod1 + sqrt(Pt(u))*h2.*xmod2 + sqrt(Pt(u))*h3.*xmod3+n1;
...
%Decode at user 2
   ...
   rem2 = eq2 - sqrt(pt(u))*dec12_remod;	%SIC to remove U1's data
...
%Decode at user 3
   ...
   rem31 = eq3 - sqrt(pt(u))*dec12_remod;	%SIC to remove U1's data
   ...
   rem3 = rem31 - sqrt(pt(u))*dec23_remod;	%SIC to remove U2's data
