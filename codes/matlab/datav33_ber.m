...
%K_vec = [3,2,1];
learn_rate = 0.01;
energyth = 1/(priority+0.001)*priority_max/timeslot;

proptstart(v)=tic;

[convergedukfin,nbiterationslam,lam,nbiterationsuk,opt_decision_uk] = ...
    grad_descent_uklam(grad_lam,...
    lam,learn_rate,n_iter,tolerance,K_vec,opt_decision_uk,K,sym_dur_vec,energyth);
...
function [convergedukfin,nbiterationslam,lam,nbiterationsuk,decision_uk] =...
    ...
    grad_uk = (-K_vec-lam...
              + 2*(lam*decision_uk) - mew*(sum(sym_dur_vec) - 5));
    ...
    if (abs(sum(diffuk))<= toleranceuk)
        ...
        while(convergeduk == true)
            ...
            if (abs(diff)<= tolerance)
                ...
                convergedlam = true;
                %convergeduk = false; 
                grad_mew = (sym_dur_vec(1:K,1)'*decision_uk) - 5;
                diffmew = -learn_rate*0.1*grad_mew;
                fprintf('here %f %f\n', lam, decision_uk)
                ...
                if(abs(diffmew)<=tolerance)
                    ...
                    if ((decision_uk) <0.1*ones(K,1))
                         decision_uk = 0.1*ones(K,1);
                    end
