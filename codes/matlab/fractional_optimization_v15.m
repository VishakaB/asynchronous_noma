...
p1 =10*tx_pow_k(1);%nearest user#different transmission powers #change this
p2 =0.5*tx_pow_k(2);
p3 =0.25*tx_pow_k(3);
p4 =0.1*tx_pow_k(4);%farthest user
p5 =0.01*tx_pow_k(5);
...
for k =1:K%interference %only from the next neighbor user
    if k ~= desired_id & k >= (desired_id+1) %interference caused by weak users only
