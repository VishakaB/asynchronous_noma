...
receive_pow_ratio_vec = linspace(1,5,10);
u = 1;%initiate u
nbiter = 1;

for receive_pow_ratio = 1: length(receive_pow_ratio_vec)
K = 10;
% ber_1 = zeros(K,1);
% ber_2 = zeros(K,1);
% ber_3 = zeros(K,1);
...
%change here
for d = 2: K
    power_vec(d) = power_vec(d-1)/10^(receive_pow_ratio_vec(receive_pow_ratio));
end
clear delta_mat;
...
%% strongest user
    ...
    %% weakest user
    delta_i = [0.5*(sum(sumsym_dur_vec(end,1:initialK-1)));...
        0.5*(sum(sumsym_dur_vec(end,1:initialK)))];%known
...
%% weakest user
delta_i = [0.5*(sum(sumsym_dur_vec(end,1:kopt-1)));...
    0.5*(sum(sumsym_dur_vec(end,1:kopt)))];%known
p_d     = power_vec(end); %desired power
p_iw    = power_vec(2);   %interferes power 
p_iw1   = power_vec(1);
...
u = u+1;
% %update K  
% nonk1 = find(ber_1,1,'last');
% nonk2 = find(ber_2,1,'last');
% nonk3 = find(ber_3,1,'last');
% % avg_convber_1(j) = ber_1(nonk1);
% % avg_convber_2(j) = ber_2(nonk2);
% % avg_convber_3(j) = ber_3(nonk3);
end
...
avgavgpber_1(receive_pow_ratio) = mean(avg_pber_1);
avgavgpber_2(receive_pow_ratio) = mean(avg_pber_2);
avgavgpber_3(receive_pow_ratio) = mean(avg_pber_3);

avgavgber_1(receive_pow_ratio) = mean(avg_ber_1);
avgavgber_2(receive_pow_ratio) = mean(avg_ber_2);
avgavgber_3(receive_pow_ratio) = mean(avg_ber_3);
...
figure (4)
semilogy(receive_pow_ratio_vec,smooth(avgavgpber_1),'bo-','LineWidth',1)
hold on 
semilogy(receive_pow_ratio_vec,smooth(avgavgber_1),'bo--','LineWidth',2)
hold on
semilogy(receive_pow_ratio_vec,smooth(avgavgpber_2),'rs-','LineWidth',1)
hold on 
semilogy(receive_pow_ratio_vec,smooth(avgavgber_2),'rs--','LineWidth',2)
hold on 
semilogy(receive_pow_ratio_vec,smooth(avgavgpber_3),'k+-','LineWidth',1)
hold on 
semilogy(receive_pow_ratio_vec,smooth(avgavgber_3),'k+--','LineWidth',2)