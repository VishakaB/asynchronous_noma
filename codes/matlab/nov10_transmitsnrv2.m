...
transmit_snrdb_vec = linspace(1,2,5);
u = 1;%initiate u
nbiter = 3;
...
%% strongest user
    ...
    ber_1(u) = 1/3*(pp_bita1_1 + pp_bita1_2 + pp_bita1_3)
...
avg_pber_1(j) = mean(pber_1(1));
avg_pber_2(j) = mean(pber_2(round(m/2)));
avg_pber_3(j) = mean(pber_3(end));
...
avgavgpber_1(transmit_snrdb) = mean(avg_pber_1);
avgavgpber_2(transmit_snrdb) = mean(avg_pber_2);
avgavgpber_3(transmit_snrdb) = mean(avg_pber_3);

avgavgber_1(transmit_snrdb) = mean(avg_ber_1);
avgavgber_2(transmit_snrdb) = mean(avg_ber_2);
avgavgber_3(transmit_snrdb) = mean(avg_ber_3);
...
figure (4)
semilogy(transmit_snrdb_vec,smooth(avgavgpber_1),'bo-','LineWidth',1)
hold on 
semilogy(transmit_snrdb_vec,smooth(avgavgber_1),'bo--','LineWidth',2)
hold on
semilogy(transmit_snrdb_vec,smooth(avgavgpber_2),'rs-','LineWidth',1)
hold on 
semilogy(transmit_snrdb_vec,smooth(avgavgber_2),'rs--','LineWidth',2)
hold on 
semilogy(transmit_snrdb_vec,smooth(avgavgpber_3),'k+-','LineWidth',1)
hold on 
semilogy(transmit_snrdb_vec,smooth(avgavgber_3),'k+--','LineWidth',2)
grid on;