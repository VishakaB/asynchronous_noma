...
K = 6;%number of superimposed data
initialK=K;
for nbusers = 2: initialK%number of superimposed data loop
...
energy_eff(j) = total_throughput/(total_energ_consump +0.01);

%%conv sic
total_energ_consump_conv = E_max - E_max^(exp(-(log(2)/1000)*sum(K_vec)));
energy_eff_conv(j) = total_throughput/(total_energ_consump_conv+0.01);

%% complexity analysis 
%only sic decoding 
sic_complextiyprop(j) = sum(opt_decision_uk)^2*log(1/0.01);
sic_complextiyconv(j) = sum(initialK)^2;
fprintf("print number of successive decoding j %i\n",j);
