...
transmit_snr = linspace(60,80,20);
...
etath        = 4;%change this 
noisepower   = 0.001;
max_tx_power = 10000;%change this
B            = 1;%channel bandwidth in megahertz
timeslot     = 1;
...
power_vec(1) = 10^(transmit_snrdb/10)*noisepower;%snr in linear scale*noisepower
...
h_vec =  pathloss_exp.*sqrt(power_vec(1)/2).*(randn(1,N)+1i*randn(1,N))/sqrt(2);
...
miter = 10;
priority_max = 3;%change here
lambda1 = 1;%change this%energy saving priority %left energy is low
learn_rate = 0.4;
...
for k = 1:K%neighbor users index
    ...
    if k ~= desired_id & k == desired_id+1 & desired_id == 1  %strongest user      
        sumsym_dur_vec(desired_id,1) = 2*sum(delta_mat(desired_id+1,:));      
    ...
    elseif desired_id== K%last incoming user
        sumsym_dur_vec(desired_id,1) = 2*sum(reverse_delta_mat(desired_id-1,:)); 
    end
end
...
cvx_begin quiet
   ...
   subject to
      ...
      var2: decision_uk'*sumsym_dur_vec-1/(5+0.001)*priority_max/timeslot <= 0
...
if(K>1)
    [sim_delay_prop(v),ber_prop(v)] = sim_delayfunc(K, h_vec(1:K,:), userdata_vec(1:K,:), nbrandom_iterations,K_vec);
end
if(K>1)
    [sim_delay_conv(v),ber_conv(v)] = sim_delayfunc(initialK, h_vec(1:initialK,:), userdata_vec, nbrandom_iterations,initialK_vec);
end
...
energy_eff_conv = total_throughput/(total_energ_consump_conv+0.01);
energy_eff_conv;
...
avgenergy_effconv(i) = abs((energy_eff_conv));
...
if user_strength(i) == 1 & j == 1%A1
    ...
    if (length(user_strength)>2)
       delta_i = [delta_mat(user_strength(i+1),j);...
           delta_mat(user_strength(i+2),j)];%known
       p_d     = power_vec(i); %desired power
...
elseif (user_strength(i) > 1 & j == 1 & i~=length(user_strength))%B1
    ...
    p_err_sym_ec2 = p_interb2*(ber_vec(i-1,j))*log(mod_order);
    ...
    p_err_sym_ec3 = p_interb3*(ber_vec(i-1,j+1))*log(mod_order);
    ...
    p_err_sym_ec4 = p_interb4*log(mod_order);
...
elseif (user_strength(i) > 1 & j > 1 & i~=round(length(user_strength)))%B2.....Bn & not last user 
    %disp('here1 ')
    %inter -> A2, A3 and C1, C2 %middle user
    nbinterf = 4;
    ...
    p_err_sym_ec4 = p_interb4*log(mod_order)*log(mod_order);
...
elseif (user_strength(i) > 1 & j == 1 & i==round(length(user_strength)))
    ...
    q2   = integral(fun1,timeoff_min,timeoff_max);
    p_interb2 = (1/(timeoff_max -timeoff_min))*q2*sum(delta_i.*error_vec2);
    p_err_sym_ec2 = p_interb2*(ber_vec(i-1,j))*...
        log(mod_order);
    ...
    q3   = integral(fun1,timeoff_min,timeoff_max);
    p_interb3 = (1/(timeoff_max -timeoff_min))*q3*sum(delta_i.*error_vec3);
    p_err_sym_ec3 = p_interb3*(ber_vec(i-1,j+1))*...
        log(mod_order);
...
function [berfinalconv] = berfuncconv(power_vec, noise, nsym, user_strength,...
    delta_mat,reverse_delta_mat,mod_order,timeoff_min,timeoff_max)
for i = 1: length(user_strength)
for j = 1: nsym(i)
