...
fun = @(delta_i) (qfunc(sqrt(3*p_b./(2.*(mod_order-1).*...
...
fun = @(delta_i) (qfunc(sqrt(3*p_c./(2.*(mod_order-1).*...
...
p_bitc11 = p_err_symc/log(mod_order)

p_err_symc = p_interc*(1 - p_bitb1*log(mod_order))*(p_bitb2*log(mod_order));
p_bitc12 = p_err_symc/log(mod_order)

p_err_symc = p_interc*(p_bitb1*log(mod_order))*(1-p_bitb2*log(mod_order));
p_bitc13 = p_err_symc/log(mod_order)

p_err_symc = p_interc*(1-p_bitb1*log(mod_order))*(1-p_bitb2*log(mod_order));
p_bitc14 = p_err_symc/log(mod_order)

p_bitc = p_bitc11 +p_bitc12 +p_bitc13 +p_bitc14 

%% first user BER for 2nd time iteration
% bit error rate symbol wise 
%{
%A1 -> interf by B1
delta_21 = 0.5;
...
p_err_sym1 = (1/(timeoff_max -timeoff_min))*q*delta_i*...
    ...
    (1/(timeoff_max -timeoff_min))*q*delta_i*...
    (1-p_bitb1*log(mod_order));
p_bita1 = p_err_sym1/log(mod_order)
...
fun = @(delta_i) (qfunc(sqrt(3*p_a./(2.*(mod_order-1).*...
    (delta_i.*p_b/2+noise))))).^2;
p_err_sym2 = (1/(timeoff_max -timeoff_min))*q*delta_i*(p_bitb1*log(mod_order))*(p_bitb2*log(mod_order));
p_bita2inter1 = p_err_sym2/log(mod_order)

p_err_sym2 = (1/(timeoff_max -timeoff_min))*q*delta_i*(1-p_bitb1*log(mod_order))*(p_bitb2*log(mod_order));
p_bita2inter2 = p_err_sym2/log(mod_order)

p_err_sym2 = (1/(timeoff_max -timeoff_min))*q*delta_i*(p_bitb1*log(mod_order))*(1-p_bitb2*log(mod_order));
p_bita2inter3 = p_err_sym2/log(mod_order)

p_err_sym2 = (1/(timeoff_max -timeoff_min))*q*delta_i*(1-p_bitb1*log(mod_order))*(1-p_bitb2*log(mod_order));
p_bita2inter4 = p_err_sym2/log(mod_order)

p_bita2_2iter = p_bita2inter1+p_bita2inter2+p_bita2inter3+p_bita2inter4 
