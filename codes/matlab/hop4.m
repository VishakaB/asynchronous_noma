...
recdata4C=(data_equilizedC'*code5')';
xC_hat = ones(1,N);
xC_hat(recdata4C<0) = -1;
recdata5user=(data_equilized5'*code5'-xC_hat')';
est5h4=real(recdata5user)>0;
errors_user5(i) = size(find([data_user5- est5h4]),2); %Errors for User1
SBer5h4 = errors_user5/N;   

x5user_hat = ones(1,N);
x5user_hat(recdata5user<0) = -1;
recdataC=(data_equilizedC'*code5'-x5user_hat')';
estCh4=real(recdataC)>0;
errors_userC(i) = size(find([data_userC- estCh4]),2); %Errors for User1
SBerCh4 = errors_userC/N;                              % simulated ber user1
