...
%results: ee vs total superimposed data v2
clc;
clear all;
close all;
...
f  = zeros(mpriority-1,1);
g  = zeros(mpriority-1,1);
...
[x(initialK-1),y(initialK-1),z(initialK-1),zz(initialK-1),e(initialK-1)],f(initialK-1),g(initialK-1)...
...
learn_rate = 0.4;
tolerance2 = 0.5;%lambda $change here
tolerance = 0.02;%uk%change here
Rmin = 0.000001;
sinr_th = 1e-6;
%--------------------------------------------------------------------------
...
diff  = -learn_rate*grad_lam;
    ...
    if (abs(diff) <= tolerance)
        ...
        if (isnan(decision_uk))
            ...
            if (1/K*abs(mean(diffuk)) <= tolerance2)
                ...
                iter_convguk(v) =iter_convguk(v) +1;
    ...
    iter_convglam(v) = iter_convglam(v) +1;
...
avg_iterlam(i)   = mean( iter_convglam);
avg_iteruk(i)    = mean( iter_convguk);
avgiterations(i) = mean(iterations);
fprintf('avgiterations %f\n',avgiterations);
